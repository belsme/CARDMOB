...
import {
    ...
    const CheckoutScreen = ( {navigation}: any) => {
        const { getTotalPrice, clearCart, cartItems} = useShop();
        const [phone, setPhone] = useState('');
        const [address, setAddress] = useState('');
        const [customer, setCustomer] = useState('');
        const [paymentOption, setPaytmentOption] = useState('');
        ...
        const confirmOder = async () => {
            ...
            // todo: implementar o srviço de "checkout"
            ...
            >
                <ScrollView contentContainerStyle={styles.container}>
                    ...
                    <View style={styles.paymentOption}>
                        <Button
                            title="PIX"
                            onPress={() => setPaytmentOption('PIX')}
                            color={paymentOption === 'Pix' ? '#007BFF' : '#DDD'}
                        />
                        <Button
                            title="Cartão de débito"
                            onPress={() => setPaytmentOption('Cartão de débito')}
                            color={paymentOption === 'Cartão de débito' ? '#007BFF' : '#DDD'}
                        />
                        <Button
                            title="Cartão de crédito"
                            onPress={() => setPaytmentOption('Cartão de crédito')}
